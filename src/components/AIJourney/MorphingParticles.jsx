...
const MorphingParticles = ({
  ...
  const stages = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const s0Color =
        i % 10 === 0 ? hslToRgb(24, 95, 68) : hslToRgb(0, 0, 90 - (i % 25));
      colors[0][i3] = s0Color[0];
      colors[0][i3 + 1] = s0Color[1];
      colors[0][i3 + 2] = s0Color[2];
      ...
      const s2Color =
        ringNode % 2 === 0 ? hslToRgb(190, 95, 62) : hslToRgb(230, 85, 65);
      colors[2][i3] = s2Color[0];
      colors[2][i3 + 1] = s2Color[1];
      colors[2][i3 + 2] = s2Color[2];
      ...
      const s3Color =
        i < 1000
          ...
          : hslToRgb(40, 95, 66);
      colors[3][i3] = s3Color[0];
      colors[3][i3 + 1] = s3Color[1];
      colors[3][i3 + 2] = s3Color[2];
      ...
      if (agentNode < 5) {
        ...
        const angle =
          ((agentNode - 5) / 5) * Math.PI * 2 - Math.PI / 2 + Math.PI / 5;
      ...
      const r5 =
        agentNode < 5 ? 0.05 + nextRand() * 0.28 : 0.05 + nextRand() * 0.2;
      ...
      const s5Color =
        agentNode < 5 ? hslToRgb(215, 95, 62) : hslToRgb(190, 95, 58);
      colors[5][i3] = s5Color[0];
      colors[5][i3 + 1] = s5Color[1];
      colors[5][i3 + 2] = s5Color[2];
      ...
      positions[6][i3 + 1] =
        pCenter[1] + r6 * Math.sin(phi6) * Math.sin(theta6);
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (
        ...
        if (i >= 1000) {
          ...
          if (i < 1260) {
            ...
            bz =
              orbitRadius * Math.cos(angle) * 0.5 +
              orbitRadius * Math.sin(angle) * 0.866;
          }
        } else {
      ...
      if (
        ...
        if (i % 12 === 0) {
          ...
          const routeProgress =
            (time * flowSpeed + i * 0.08) % (route.length - 1);
          ...
          bx =
            THREE.MathUtils.lerp(nStart[0], nEnd[0], segmentFactor) +
            Math.sin(i * 5.3) * 0.04;
          by =
            THREE.MathUtils.lerp(nStart[1], nEnd[1], segmentFactor) +
            Math.cos(i * 4.2) * 0.04;
          bz =
            THREE.MathUtils.lerp(nStart[2], nEnd[2], segmentFactor) +
            Math.sin(i * 3.1) * 0.04;
        }
      }
