const Experience = () => {
  ...
  return (
    <section id="experience" className="section container">
      <h2 style={{ fontSize: '2.5rem', textAlign: 'center', marginBottom: '4rem' }}>Work Experience</h2>
      
      <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem', position: 'relative' }}>
        {/* Timeline Line */}
        <div style={{
          position: 'absolute',
          left: '2rem',
          ...
          zIndex: -1
        }} className="md-hidden-timeline"></div>
        ...
        {experiences.map((exp, index) => (
          <div key={index} className="glass-card" style={{ padding: '2rem', position: 'relative' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', gap: '1rem' }}>
              <div>
                <h3 style={{ fontSize: '1.5rem', color: 'var(--accent-color)', marginBottom: '0.5rem' }}>{exp.role}</h3>
                <h4 style={{ fontSize: '1.2rem', fontWeight: 500 }}>{exp.company}</h4>
              </div>
              <div className="glass-pill" style={{ color: 'var(--text-secondary)' }}>
                {exp.period}
              </div>
            </div>
